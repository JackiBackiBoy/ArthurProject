...
bool Ray::Cast()
{
	...
	//	Vec2 tempIntersectPoint = Vec2(tempPos1.x + t * (tempPos2.x - tempPos1.x), tempPos1.y + t * (tempPos2.y - tempPos1.y));
	//}

	return true;
}

void Ray::DrawRay(const sf::Vector2f& anOrigin, const sf::Vector2f& aDirection, float aDistance, const sf::Color& aColor, sf::RenderWindow* aWindow)
{
	sf::VertexArray tempArr = sf::VertexArray(sf::PrimitiveType::LineStrip, 2);
	tempArr[0].position = anOrigin;
	tempArr[1].position = anOrigin + aDirection * aDistance;
